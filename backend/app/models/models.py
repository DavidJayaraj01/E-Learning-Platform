...
class User(Base):
    ...
    role = Column(ENUM(UserRole, values_callable=lambda x: [e.value for e in x]), nullable=False, default=UserRole.LEARNER)
...
class Course(Base):
    ...
    visibility = Column(ENUM(VisibilityType, values_callable=lambda x: [e.value for e in x]), default=VisibilityType.EVERYONE)
    access_type = Column(ENUM(AccessType, values_callable=lambda x: [e.value for e in x]), default=AccessType.OPEN)
...
class CourseEnrollment(Base):
    ...
    status = Column(ENUM(EnrollmentStatus, values_callable=lambda x: [e.value for e in x]), default=EnrollmentStatus.YET_TO_START)
...
class Lesson(Base):
    ...
    course_id = Column(Integer, ForeignKey("courses.id", ondelete="CASCADE"), nullable=False)
    title = Column(String(500), nullable=False)
    lesson_type = Column(ENUM(LessonType, values_callable=lambda x: [e.value for e in x]), nullable=False)
    description = Column(TEXT)
...
class UserLessonProgress(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    lesson_id = Column(Integer, ForeignKey("lessons.id", ondelete="CASCADE"), nullable=False)
    status = Column(ENUM(LessonStatus, values_callable=lambda x: [e.value for e in x]), default=LessonStatus.NOT_STARTED)
    completed_at = Column(TIMESTAMP, nullable=True)
    time_spent = Column(Interval, server_default=text("'0 minutes'::interval"))
...
class QuizAttempt(Base):
    ...
    status = Column(ENUM(QuizAttemptStatus, values_callable=lambda x: [e.value for e in x]), default=QuizAttemptStatus.IN_PROGRESS)
